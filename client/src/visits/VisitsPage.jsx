...
export default function VisitsPage() {
  ...
  function patchVisitStatus(item, status) {
    runOptimistic(`${status}-visit-${item.id}`, {
      apply: () => {
        ...
        const patch = (prev) =>
          prev.map((x) =>
            x.id === item.id ? { ...x, status, _optimistic: true } : x,
          );
        setVisits(patch);
        setVisitList(patch);
        return snapshot;
  ...
  async function handleVisitSave(data) {
    ...
    const optimisticVisit = buildOptimisticVisit(data, clinicians, patients);
        ...
        {loading ? (
          ...
          <>
            <VisitList
              ...
              onEdit={(item) => setVisitModal({ mode: "edit", item })}
              ...
              onComplete={(item) =>
                askConfirm({
                  ...
                  cancelLabel: "Not Yet",
                  intent: "success",
                  action: () => patchVisitStatus(item, "completed"),
                })
              }
              onCancel={(item) =>
                askConfirm({
                  ...
                  intent: "warning",
                  action: () => patchVisitStatus(item, "cancelled"),
                })
              }
              onDelete={(item) =>
                askConfirm({
