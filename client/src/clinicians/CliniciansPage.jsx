...
export default function CliniciansPage() {
  ...
  const pagination = useFilteredPagination(clinicians, filterClinician, search);
      ...
      action: () =>
        runOptimistic(`delete-clinician-${item.id}`, {
          apply: () => {
            ...
            setClinicians((prev) => prev.filter((x) => x.id !== item.id));
            setVisits((prev) => removeVisitsForClinician(prev, item.id));
            setVisitList((prev) => removeVisitsForClinician(prev, item.id));
            return snapshot;
          },
  ...
  async function handleSave(data) {
    ...
    await runOptimistic(`edit-clinician-${item.id}`, {
      apply: () => {
        ...
        setVisits((prev) => patchVisitsForClinician(prev, item.id, data));
        setVisitList((prev) => patchVisitsForClinician(prev, item.id, data));
        return snapshot;
      },
      ...
      onSuccess: (updated) => {
        ...
        setVisits((prev) => patchVisitsForClinician(prev, updated.id, updated));
        setVisitList((prev) =>
  ...
  return (
    <>
      ...
      {modal && (
        <ClinicianFormModal
          title={modal.mode === "add" ? "Add New Clinician" : "Edit Clinician"}
          initial={modal.item}
